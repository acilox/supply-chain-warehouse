...
def test_zscore_detection():
    ...
    readings = [
        TelemetryReading(device_id="D", timestamp=base_time + timedelta(seconds=i), temperature_c=5.0)
        for i in range(10)
    ]
    readings.append(TelemetryReading(device_id="D", timestamp=base_time + timedelta(seconds=60), temperature_c=50.0))
